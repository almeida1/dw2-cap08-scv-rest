...
public class PedidoServicoI implements PedidoServico {
	...
	public boolean isCadastrado(String cpf) {
		...
		try {
			...
			logger.info(">>>>>> servico isCadastrado chamado - login com sucesso" + headers.toString());
			HttpEntity<Cliente> httpEntity2 = new HttpEntity<Cliente>(headers);
			RestTemplate template = new RestTemplate();
			ResponseEntity<Cliente> resposta = null;
			String url = "https://dw-scc-rest.herokuapp.com/api/v1/clientes/{cpf}";
			resposta = template.exchange(url, HttpMethod.GET, httpEntity2, Cliente.class, cpf);
			return true;
